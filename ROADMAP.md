...
- **E1 NULL (D-029) → E1.5/D-030 DONE: the 2-engine entry-divert re-authorization LIFTS EO recovery** (reactive 0/1→**9/10**, neural 1/0/0→**8/4/2** of 60; byte-clean). Pivotal: reactive now beats neural on EO (clean-trained policy mishandles the 2-engine handoff). **NEXT BOX = E2: EO DAgger — retrain the policy on the improved-entry-divert EO handoff → NP_VERSION 7.**
...
- ⬜ **E2 — ENTRY engine-out DAgger rounds** (now unblocked by D-030): teacher = the best EO controller with D-030 (reactive/MPPI ~9–10/60) + on-policy shadow → merged retrain → **NP_VERSION 7** → KAT ceremony (C pass) → full gates + no-regression floors (AERO 46 / gust 45 / ENTRY-clean ≥57) → ENTRY `--engine-out random` ×60 ×3 eval, scored recovery-vs-frontier (D-027 oracle). Retrain the policy to handle the EO handoff (the D-030 gap). Expect a ladder like ENTRY clean's 0→52→57.
