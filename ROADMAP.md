...
- **E1 DONE (D-029): composite is a NULL** (1/60 == student == teacher; not an improvement operator). Phase-attribution ⇒ bottleneck is the **2-engine ENTRY DIVERT** (gross cluster lost at the burn cut, ~830 m closed). **NEXT BOX = E1.5 (D-030): re-authorize the 2-engine entry divert — open the low-qbar bank cap + re-tune gains under n_eng<3.**
...
- ⬜ **E1.5 — 2-ENGINE ENTRY-DIVERT re-authorization (D-030): the real EO lever.** `entry_divert_step` (ZEM/ZEV KR=2/KV=3.5, bank cap 15°, `amax=n_eng·thrust·sin(15°)/m`) loses 1/3 authority under engine-out. Entry burn is LOW-qbar (~0.2–40 kPa vs 80 kPa STRUCT) ⇒ OPEN the bank cap + re-tune gains/t_go when n_eng<3 (byte-clean: n_eng==3 untouched ⇒ every golden reproduces). Measure recovery-vs-frontier (D-027: D_phys_2eng ≥12,656 m ⇒ closure IS available; the +22.9 m/s outbound-at-cut signature = under-driven/mistimed). D-012-style sweep; ship D-030.
