# D-061 — THE HEADLINE, VERIFIED ONCE ON A SEALED POOL.
#
...
$dir  = "C:/Booster_Lander_Simulator/runs/d061"
$out  = "C:/Booster_Lander_Simulator/runs/d061_sealed_verify.txt"
$seeds = 9300..9309
New-Item -ItemType Directory -Force -Path $dir | Out-Null

Add-Type -Name Pwr -Namespace W61 -MemberDefinition @'
[DllImport("kernel32.dll", SetLastError=true)]
public static extern uint SetThreadExecutionState(uint esFlags);
'@
$ES_CONTINUOUS = [uint32]2147483648
[void][W61.Pwr]::SetThreadExecutionState([uint32]($ES_CONTINUOUS -bor [uint32]1))
...
"D-061 SEALED 9300-9309, flown ONCE: the E6 cold 1/32 row (179/180 dev, ~1 s/flight)" | Out-File $out
...
$arms = @(
  @{ name = 'blind+event+1of32'; args = @('--rfly-blind','--rfly-event-replan','--rfly-budget','0.03125') }
)
...
foreach ($arm in $arms) {
    ...
    foreach ($s in $seeds) {
        ...
        if (-not (Select-String -Path $res -Pattern "LANDED:" -Quiet)) {
            ...
            "D061-FAILED $($arm.name) seed $s" | Out-File $out -Append
            $ok = $false; break
        }
...
if ($ok) { "D061-DONE  $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')" | Out-File $out -Append }
[void][W61.Pwr]::SetThreadExecutionState($ES_CONTINUOUS)
