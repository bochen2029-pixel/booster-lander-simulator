# D-052 — EVENT-TRIGGERED REPLAN AT FULL BUDGET: is the stale plan the LOC mechanism?
#
...
$dir = "C:/Booster_Lander_Simulator/runs/d052"
$out = "C:/Booster_Lander_Simulator/runs/d052_event_replan.txt"
...
"D-052 EVENT-TRIGGERED REPLAN at FULL budget, blind — is the stale plan the LOC mechanism?" | Out-File $out
...
foreach ($s in 42, 7, 99) {
    ...
    & $exe --headless --scenario entry --seed $s --runs 60 --rfly --rfly-blind `
           --rfly-event-replan --engine-out random 1> $res 2> $err
    ...
    if (-not (Test-Path $res) -or -not (Select-String -Path $res -Pattern "LANDED:" -Quiet)) {
        ...
        "D052-FAILED seed $s" | Out-File $out -Append; $ok = $false; break
    }
    "===== seed $s ($mins min) =====" | Out-File $out -Append
    Select-String -Path $res -Pattern "LANDED:|PERFECT|faults:|crash causes:|landed means:" |
        ForEach-Object { $_.Line.Trim() } | Out-File $out -Append
...
if ($ok) { "D052-DONE  $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')" | Out-File $out -Append }
